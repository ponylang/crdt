...
class ref MVHashReg[A: Equatable[A] val, H: HashFunction[A] val]
  ...
  """
  A mutable register that retains all concurrently written values. When
  replicas write different values without first converging, all values are
  kept until a subsequent update overrides them. After convergence, the
  register exposes the full set of concurrently written values.
